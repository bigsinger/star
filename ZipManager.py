...
class ZipManager:
    ...
    def delFileFromZip(tool_path, apk_path, file2del):
        ...
        if len(result) > 0:
            logging.error(u"[delFileFromZip] %s", result)
            return False
        else:
            return True
    ...
    def addFileToZip(tool_path, apk_path, file_path, file_name):
        ...
        result = star.runExe(command)
        if len(result) > 0:
            logging.error(u"[addFileToZip] %s", result)
            logging.error(u"[addFileToZip] 向zip中添加文件错误 FileEntry:%s Path:%s", file_name, file_path)
            return False
        else:
            return True
    ...
    def addDirToZip(tool_path, apk_path, dir_name):
        ...
        result = star.runExe(command)
        if len(result) > 0:
            logging.error(u"[addDirToZip] 向%s中添加目录%s失败，输出%s", apk_path, dir_name, result)
            return False
        return True
    ...
    def createZip(zipPath, compressionFlag=zipfile.ZIP_STORED):
        ...
        if not os.path.exists(zipPath):
            logging.error(u"[createZip] 压缩文件%s不存在", zipPath)
            return False
        return True
